...
def spice_troops_at_safe_distance(cities, distance):
    ...
    if safe_distance >= 0:
        ...
        for city_name in safe_distance:
...
def resolve_production(board, cities, own_empire, harvest):
    ...
    for distance in distances:
        ...
        if distance_if_fully_owned:
            spice_from_spiced_troops += spice_troops_at_safe_distance(cities, distance)
        else:
            resolve_troops_placing(cities, distances, distance, harvest)
            break
